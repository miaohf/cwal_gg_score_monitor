...
func (s *windowsOverlayState) stop() {
	s.closeOnce.Do(func() {
		close(s.stopCh)
	})
}
...
func (s *windowsOverlayState) applyLayeredWindowAttributes() {
	...
	alpha := s.opacityA
	s.opacityMu.RUnlock()
	_, _, _ = procSetLayeredWindowAttrs.Call(
		s.hwnd,
		0x00000000,
		uintptr(alpha),
		uintptr(lwaColorKey|lwaAlpha),
	)
}
...
func (s *windowsOverlayState) adjustOpacity(delta int) {
	...
	if s.prefs != nil {
		// Reuse the same preference key as BG Transparency so Windows native mode
		// and legacy Fyne mode share one opacity setting.
